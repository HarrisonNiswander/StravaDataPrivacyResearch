...
output_path = os.path.join(script_dir, 'clusterPred_DistanceData', f'distanceAthlete{athlete}.csv')

#import athlete data
df = pd.read_csv(athlete_data_path, on_bad_lines='skip')

#obtain distance
df['distance'] = pd.to_timedelta(df['distance'], unit='s')
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):
    ...
    for i in range(len(X_test)):
        ...
        # Add distance features for this specific test sample
        result['distance'] = data.iloc[test_idx[i]]['distance']
