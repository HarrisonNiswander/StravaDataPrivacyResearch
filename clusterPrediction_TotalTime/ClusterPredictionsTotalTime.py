...
output_path = os.path.join(script_dir, 'clusterPred_TotalTimeData', f'totalTimeAthlete{athlete}.csv')

#import athlete data
df = pd.read_csv(athlete_data_path, on_bad_lines='skip')

#obtain elapsed time
df['elapsed_time'] = pd.to_timedelta(df['moving_time'], unit='s')

#convert elapsed time into cyclic features
df['hour_sin'] = np.sin(2 * np.pi * df['elapsed_time'].dt.total_seconds() / (24 * 3600))
df['hour_cos'] = np.cos(2 * np.pi * df['elapsed_time'].dt.total_seconds() / (24 * 3600))
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):
    ...
    for i in range(len(X_test)):
        ...
        result['elapsed_time'] = data.iloc[test_idx[i]]['elapsed_time']
